...
the distribution that a buyer actually pays, and it does so in about one
sixty-fifth of the time the autoregressive model needs; every difference is
statistically significant. Its intervals are nevertheless too narrow: the
...
quantiles over the horizon, measured on a single core. The analog search is
implemented as a plain loop over candidate windows; a vectorized search would
reduce the figures reported here, so they should be read as an upper bound on
...
\midrule
\input{tables/calibration_results}
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\input{tables/sensitivity_results}
\bottomrule
\end{tabular}
\end{table}
...
combination of the ranking with the cost column. A method that produces a
calibrated distribution in a fraction of a second can be run for every node of
